...
from emotion_p import preprocess_sentence


def buildtoken():
    tokenizer = tfds.deprecated.text.SubwordTextEncoder.load_from_file('models/scripts/previous/tokenizer.tf')
...
tokenizer, START_TOKEN,END_TOKEN,VOCAB_SIZE=buildtoken()
#transformer_model= load_model('models/scripts/t_model/transformer.h5')
MAX_LENGTH = 40
...
def evaluate(sentence,emotion):
    ...
    transformer_model.load_weights('models/scripts/previous/mymodel.h5')
    ...
    for i in range(MAX_LENGTH):
        predictions = transformer_model(inputs=[sentence,emotion, output], training=False)
...
def predict(sentence,emotion):
    prediction,tokenizer = evaluate(sentence,emotion)
...
#print(predict("i am happy",'neutral'))